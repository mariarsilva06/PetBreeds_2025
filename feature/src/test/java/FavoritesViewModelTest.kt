...
class FavoritesViewModelTest {
    ...
    @Test
    fun `GIVEN favorite pets WHEN toggleFavorite is called THEN removes pet from favorites list`() = runTest {
        // Given
        ...
        val initialState = FavoritePetsState.Success(
            pets = initialPets,
            favoritesCount = 2f,
            averageLifespan = 12f
        )

        val updatedState = FavoritePetsState.Success(
            pets = updatedPets,
            favoritesCount = 1f,
            averageLifespan = 13f
        )
    ...
    @Test
    fun `GIVEN favorite dogs WHEN pet type changes to DOG THEN loads dog favorites`() = runTest {
        // Given
    ...
    @Test
    fun `GIVEN pets with specific lifespans WHEN calculating average THEN returns mathematically correct value`() = runTest {
        // Given
        val favoritePets = listOf(
    ...
    @Test
    fun `GIVEN single favorite pet WHEN toggleFavorite is called THEN transitions to empty state`() = runTest {
        // Given
    ...
    @Test
    fun `GIVEN pets with invalid lifespan WHEN calculating average THEN handles gracefully`() = runTest {
        // Given
        val favoritePets = listOf(
    ...
    @Test
    fun `GIVEN new pet type WHEN setPetType is called THEN calls preferences manager`() = runTest {
        // Given
    ...
    @Test
    fun `GIVEN DOG favorites exist WHEN pet type changes to DOG THEN loads dog favorites`() = runTest {
        // Given
        val catPets = listOf(createMockPet("1", "Persian", isFavorite = true))
        val dogPets = listOf(
            createMockPet("2", "Labrador", isFavorite = true).copy(petType = PetType.DOG)
